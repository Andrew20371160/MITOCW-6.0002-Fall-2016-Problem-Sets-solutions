...
class Position(object):
    ...
    def __str__(self):  
        return "Position: " + str(math.math.floor(self.x)) + ", " + str(math.math.floor(self.y))
...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for i in range(self.height):
            for j in range(self.width):
                self.tiles[(i,j)] = int(dirt_amount)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        if self.tiles[(math.floor(pos.get_x()),math.floor(pos.get_y()))] <=capacity:
            self.tiles[(math.floor(pos.get_x()),math.floor(pos.get_y()))] = 0
        else:
            self.tiles[(math.floor(pos.get_x()),math.floor(pos.get_y()))] -=int(capacity)
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        return Position(float(random.randint(0,self.height)),float(random.randint(0,self.width)))
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        return RectangularRoom.is_position_in_room(self,pos) and not self.is_position_furnished(pos)
    ...
    def get_random_position(self):
        ...
        for item in self.tiles:
            if not self.is_tile_furnished(item[0],item[1]):
                temp_available.append(item)

        ret_t = random.choice(temp_available)

        return Position(float(ret_t[0]),float(ret_t[1])) 
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_pos = curr_pos.get_new_position(angle =new_dir ,speed = self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        cleaned= room.get_num_cleaned_tiles()

        while (cleaned/room.get_num_tiles())<min_coverage:
            ...
            for robot in robots:
                ...
                if (cleaned/room.get_num_tiles())>=min_coverage:
                    break


        room = EmptyRoom(width,height,dirt_amount)
        for i in range(num_robots):
            robots[i]=robot_type(room,speed,capacity)
